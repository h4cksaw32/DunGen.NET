...
namespace DunGen.NET
{
    public class Grid
    {
        ...
        public byte?[,] GetArea(int x, int y)
        {
            byte?[,] result = new byte?[3, 3];
            result[1, 1] = GetTile(x, y);
            result[0, 0] = x > 0 && y > 0 ? GetTile(x - 1, y - 1) : null;
            result[0, 1] = y > 0 ? GetTile(x, y - 1) : null;
            result[0, 2] = x < width - 1 && y > 0 ? GetTile(x + 1, y - 1) : null;
            result[1, 0] = x > 0 ? GetTile(x - 1, y) : null;
            result[1, 2] = x < width - 1 ? GetTile(x + 1, y) : null;
            result[2, 0] = x > 0 && y < height - 1 ? GetTile(x - 1, y + 1) : null;
            result[2, 1] = y < height - 1 ? GetTile(x, y + 1) : null;
            result[2, 0] = x < width - 1 && y < height - 1 ? GetTile(x + 1, y + 1) : null;
            return result;
        }
    }
}
